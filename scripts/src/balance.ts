 ...
 *
 * 准备：在项目根目录 .env 中设置 FUJI_PRIVATE_KEY（可先用测试网水龙头领测试 AVAX）
 * 运行：pnpm sdk:balance   （或 npm run sdk:balance）
 *
...
import { formatEther } from "@avalanche-sdk/client/utils";
...
async function main() {
  // 创建客户端并查询 C-Chain 上的原生 AVAX 余额（viem getBalance）
  ...
  // 场景 A：有私钥 → 派生账户，展示 EVM + X/P 地址并查余额
  if (PRIVATE_KEY && !PRIVATE_KEY.replace(/^0x/, "").match(/^0+$/)) {
  ...
  if (CHECK_ADDRESS) {
    console.log("\n查询地址:", CHECK_ADDRESS);
    const balanceWei = await client.getBalance({ address: CHECK_ADDRESS as `0x${string}` });
  ...
  console.error(
    "\n未配置查询目标。任选其一：\n" +
      "  1) 在根目录 .env 设置 FUJI_PRIVATE_KEY（账户+余额演示）\n" +
